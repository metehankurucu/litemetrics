...
Filtering is enabled by default (`BOT_FILTER_MODE=standard`): Layer 1 drops the event, Layers 2 and 3 keep it with a `bot_flag` and hide it from queries. It is per-site overridable from the dashboard Settings page. All read endpoints accept `?includeBots=true` to surface flagged events. See [Self-Hosting](./self-hosting.md#bot-filtering) for mode details.
