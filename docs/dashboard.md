...
- A bot-traffic pill appears when the filter has flagged events in the current window, broken down by layer: `signature`, `heuristic`, `rate-limit`, `velocity` (flagged events are kept; tick *Include bot traffic* to see them). Dropped events are never stored, so they are never counted here
- Export data as CSV
