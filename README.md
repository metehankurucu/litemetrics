...
- **Layer 3 (rate limit)** — sliding-window per-IP cap (`BOT_RATE_WINDOW_MS` / `BOT_RATE_MAX`) for traffic that escapes the other three layers. It is evaluated last, so a request flagged by Layer 4 never reaches it. The window counts collect requests, not events, and is shared across every site served by the process, so one IP browsing two of your sites reaches the cap twice as fast.
- **Layer 4 (visitor velocity)** — sliding-window cap on how many **pageviews** a single `visitorId` may send (`BOT_VELOCITY_WINDOW_MS` / `BOT_VELOCITY_MAX_PAGEVIEWS`, default 60 per 10 seconds). The three layers above all read one request in isolation: two read headers, and the per-IP window counts *requests*, so a batched flood arrives as a handful of calls from an address that rotates. This layer measures the visitor instead. Only pageviews count toward it — rage clicks and scroll-depth events fire dozens of times a minute by design — and the window is per `siteId:visitorId`, so it is keyed to a visitor rather than to an address: a shared NAT is not pooled by its IP. **A `visitorId` is a device fingerprint for one day, not a person.** The browser tracker derives it from `hostname|day|User-Agent|language|timezone|screen` (`packages/tracker/src/session.ts`), and four of those six carry no cross-visitor entropy, so two people on the same office build, locale and screen size send the same id and DO share one window: about 6 pageviews a second between them reaches the threshold. The React Native SDK is not affected, it draws a random per-install id. If your visitors are clustered like that on a URL-syncing page, raise `BOT_VELOCITY_MAX_PAGEVIEWS` or run `shadow` first; `strict` would drop those rows.
