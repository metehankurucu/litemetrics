...
| `BOT_VELOCITY_MAX_PAGEVIEWS` | Max **pageviews** one visitor may send per window before the velocity layer fires. `0` switches this layer off on its own | `60` |
| `BOT_VELOCITY_MAX_KEYS` | Cap on tracked `siteId:visitorId` windows (LRU-evicts the oldest) | `50000` |
...
- **Layer 3 (rate limit)** — sliding-window per-IP cap (`BOT_RATE_WINDOW_MS` / `BOT_RATE_MAX`) for traffic that escapes Layers 1 and 2. It is evaluated **before** Layer 4, and deliberately: Layer 4 judges one visitor and stores the rest of the batch, so a request it flags still lands rows, and a request that lands rows has to be counted against its address. A Layer 1 or Layer 2 hit does skip this window, because neither ever admits an **unflagged** row: either the request returns and nothing is stored, or the whole batch is stored carrying the flag. Layer 4 is the one that stores unflagged rows alongside a hit, which is why it may not skip. The window counts collect requests, not events, and is shared across every site served by the process, so one IP browsing two of your sites reaches the cap twice as fast. With `TRUST_PROXY` on (the default), the per-IP key is the first `X-Forwarded-For` entry, falling back to `X-Real-IP`. Unless the proxy in front of the collector overwrites `X-Forwarded-For`, that entry is whatever the client sent: a client can rotate it to get a fresh Layer 3 window on every request, or set it to someone else's address to spend that address's budget, which under `standard` hides the address's traffic from the default reports — the per-IP counterpart of the forged-`visitorId` case below. Some hosts document only `X-Real-IP` as the client address, Railway among them; check what yours sets.
- **Layer 4 (visitor velocity)** — sliding-window cap on how many **pageviews** a single `visitorId` may send (`BOT_VELOCITY_WINDOW_MS` / `BOT_VELOCITY_MAX_PAGEVIEWS`, default 60 per 10 seconds). Once a visitor is over the line, its pageviews stay flagged while it keeps above that rate, and it clears one window after it slows. The three layers above all read one request in isolation: two read headers, and the per-IP window counts *requests*, so a batched flood arrives as a handful of calls from an address that rotates. This layer measures the visitor instead. Only pageviews count toward it — rage clicks and scroll-depth events fire dozens of times a minute by design — and the window is per `siteId:visitorId`, so it is keyed to a visitor rather than to an address: a shared NAT is not pooled by its IP. **A `visitorId` is a device fingerprint for one day, not a person, and the client sends it.** The browser tracker derives it from `hostname|day|User-Agent|language|timezone|screen` (`packages/tracker/src/session.ts`), which is a hash of things two people can easily share: same site, same day, same browser build, same locale, same timezone, same screen size. Two such visitors send the *same* id and DO share one window, so about 6 pageviews a second between them reaches the threshold. Because the id travels in the request body and its inputs are observable, it can also be **forged**: anyone can compute a common visitor's id and burn that window deliberately, which in `standard` hides that visitor's next pageviews from the default reports and in `strict` drops them. Keying the window on the IP as well would close that, and would also hand the batching, address-rotating client this layer exists for a free pass, so it is not done. The React Native SDK is unaffected either way: it draws a random per-install id. If your own visitors cluster like that on a URL-syncing page, raise `BOT_VELOCITY_MAX_PAGEVIEWS` or run `shadow` first. Once a visitor is over the line, **all** of its events in that batch are acted on, custom events included; only pageviews *count* toward the window.

**App sites (`type: 'app'`) never run Layers 1 or 2.** Those are browser heuristics — a native app SDK sends no browser User-Agent, no `Accept-Language` and no `Referer`, so on an app site they only ever misfire (on Android, React Native's `fetch` goes out as `okhttp/<version>`, which `isbot` matches, and every Android event was being dropped). In `standard`, an app site runs **Layer 4 only**: the SDK's 5-second send timer and carrier NAT put many devices on one public address, so the per-IP window would hide real users once a handful of devices shared an address. `strict` and `shadow` still run Layer 3 too, alongside Layer 4 — dropping (`strict`) or flagging (`shadow`) an over-limit request. Layer 4 is not a browser heuristic — it measures how fast one visitor moves — so it applies to app traffic the same way the per-IP window does on web sites. This makes the site type load-bearing: **a site that receives app SDK traffic must be created with `type: 'app'`** (`litemetrics sites create --type app`, or `POST` / `PUT /api/sites` with `{"type":"app"}`), otherwise it is still filtered as browser traffic, and that now costs more than it used to. The SDK's own `litemetrics-react-native/<version> (<platform>)` User-Agent escapes Layer 1, but it resolves to no browser and no engine, and the SDK sets no `Accept-Language` or `Referer`. On Android nothing below the SDK adds them (OkHttp does not), so Layer 2 flags the request: on a mis-typed site `standard` hides that Android traffic from every report (recoverable with `?includeBots=true`) and `strict` drops it outright. iOS has not been measured: its network stack (`NSURLSession`) may add `Accept-Language` on its own, in which case iOS requests do not trip Layer 2 and a mis-typed site shows its iOS traffic while hiding its Android traffic. The server logs `[site-type-mismatch] site=<id> type=<type> platform=<platform> mode=<mode>` once per site when it sees app SDK payloads on a non-app site; the payload alone never bypasses the filter.

Modes are configured server-wide via `BOT_FILTER_MODE` and overridable per-site (on `app` sites Layer 4 always applies; Layer 3 joins it in `strict` and `shadow`, see above):
...
```
[bot-filter] dropped layer=signature reason=ua-signature mode=standard events=1 site=site_abc ip=203.0.113.4 ua="okhttp/4.12.0"
```
...
| `velocity` | velocity | One `visitorId` sent more pageviews inside the velocity window than a person can read |
...
| `bot_dropped` / `bot_flagged` | Bot-filter outcomes, counted per **request**; these totals survive even after the individual detail lines age out of the window. A Layer 4 drop is per visitor, so one `bot_dropped` can mean part of a batch went and the rest was stored: the per-hit `events=` field on the detail line below is what says how much |
