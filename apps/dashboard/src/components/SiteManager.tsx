...
function BotFilterModeSetting({ mode, siteType, onUpdate, saving }: {
  ...
  const options: Array<{ value: 'off' | 'standard' | 'strict' | 'shadow'; label: string; hint: string }> = isApp
    ...
    : [
        { value: 'off',      label: 'Off',      hint: 'No bot filtering. Useful for testing.' },
        { value: 'standard', label: 'Standard', hint: 'Drop known bots (isbot signatures); flag scrubbed-UA, per-visitor velocity and rate-limit hits: kept, but hidden from reports. Default.' },
        { value: 'strict',   label: 'Strict',   hint: 'Drop known bots + scrubbed-UA + per-visitor velocity + rate-limit hits.' },
